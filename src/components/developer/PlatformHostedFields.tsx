...
const TS_EXAMPLE = `async function handle(input: { message: string; user_id: string; context: Record<string, unknown> }) {
  // Your agent logic here
  return { response: "Hello from my agent!", tools_used: [] };
}`;

const PY_EXAMPLE = `def handle(input: dict) -> dict:
...
export const PlatformHostedFields = ({
  ...
  const fileAccept = isTs ? ".ts,.zip" : ".py,.zip";
  const fileLabel = isTs ? "Agent Code File (.ts or .zip) *" : "Agent Code File (.py or .zip) *";
  ...
  return (
    <>
      <div className="rounded-lg border border-primary/20 bg-primary/5 p-4 space-y-2">
        ...
        <pre className="text-xs bg-muted/50 rounded p-2 font-mono overflow-x-auto whitespace-pre-wrap">
          {isTs ? TS_EXAMPLE : PY_EXAMPLE}
        </pre>
      </div>
