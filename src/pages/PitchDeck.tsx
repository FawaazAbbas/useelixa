...
import { Download, ArrowLeft, Users, TrendingUp, Target, Zap, Shield, Globe, BarChart3, CheckCircle2, Rocket, ArrowRight, ChevronRight, Clock, DollarSign, Layers, Brain, Lock, Eye, Workflow, PieChart, Check, X } from "lucide-react";
import { useNavigate } from "react-router-dom";
import { ElixaLogo } from "@/components/ElixaLogo";
import { motion } from "framer-motion";
import { SlideProgressIndicator } from "@/components/pitch-deck/SlideProgressIndicator";
import { fadeInUp, staggerContainer, scaleIn, slideInLeft, slideInRight } from "@/components/pitch-deck/slideAnimations";
...
>>>>>>> theirs
  ...
  useEffect(() => {
    ...
    slideSectionsRef.current = Array.from(
      deckRef.current.querySelectorAll("section")
    ) as HTMLElement[];
      ...
      sections.forEach((section) => {
        ...
        const scale =
          contentHeight > availableHeight
            ? Math.max(0.7, availableHeight / contentHeight)
            : 1;
    ...
    const handleScroll = () => {
      ...
      const activeIndex = sections.findIndex(
        (section) => scrollPosition < section.offsetTop + section.offsetHeight
      );
      const resolvedIndex =
        activeIndex === -1 ? sections.length - 1 : activeIndex;
...
>>>>>>> theirs
    ...
    if (typeof ResizeObserver !== "undefined") {
      ...
      slideSectionsRef.current.forEach((section) =>
...
>>>>>>> theirs
  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      <div
        ref={deckRef}
        className="pitch-deck-wrapper print:p-0"
      ...
      >
        ...
        <section className="min-h-screen flex flex-col justify-center items-center relative overflow-hidden print:break-after-page pt-20 bg-gradient-to-br from-blue-50 via-white to-indigo-50">
          ...
          >
            ...
            <motion.h1 variants={fadeInUp} className="text-7xl md:text-8xl font-bold tracking-tight text-foreground">ELIXA</motion.h1>
            
            <motion.p variants={fadeInUp} className="text-2xl md:text-3xl text-muted-foreground max-w-4xl mx-auto leading-relaxed font-light">
              A workspace where businesses <span className="text-primary font-semibold">hire AI agents</span> to run functions—marketing, ops, finance, and more.
            </motion.p>
            ...
            <motion.div variants={staggerContainer} className="grid md:grid-cols-3 gap-6 mt-12 max-w-4xl mx-auto">
              {[
                ...
                <motion.div key={i} variants={scaleIn} className="bg-card/80 backdrop-blur-sm border border-border rounded-2xl p-6 shadow-sm hover:shadow-md transition-all">
        ...
        <section className="min-h-screen flex flex-col justify-center py-20 px-6 relative print:break-after-page bg-gradient-to-br from-rose-50 via-white to-orange-50">
          ...
          >
            <motion.div variants={fadeInUp} className="space-y-4">
              ...
              <h2 className="text-5xl md:text-6xl font-bold text-foreground">Scaling is <span className="text-rose-500">broken</span></h2>
            </motion.div>
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <motion.div variants={slideInLeft} className="space-y-4">
                ...
                {[
                  { icon: Layers, title: "Tool Sprawl", desc: "15+ disconnected SaaS tools, none talking to each other" },
                  { icon: DollarSign, title: "Hiring Cost", desc: "£50k+ per hire, 3-month ramp time, constant turnover" },
                  { icon: Zap, title: "Execution Bottlenecks", desc: "Founders stuck doing ops instead of strategy" },
                  { icon: X, title: "Inconsistent Output", desc: "Quality varies by who's available, not what's needed" }
                ].map((item, i) => (
                  <div key={i} className="flex items-start gap-4 bg-card border border-rose-200 rounded-xl p-5 hover:border-rose-300 transition-all shadow-sm">
                    <div className="p-2 bg-rose-100 rounded-lg"><item.icon className="w-5 h-5 text-rose-500" /></div>
                    <div><div className="text-foreground font-semibold">{item.title}</div><div className="text-muted-foreground text-sm">{item.desc}</div></div>
                  </div>
                ))}
              </motion.div>
        ...
        <section className="min-h-screen flex flex-col justify-center py-20 px-6 relative print:break-after-page bg-gradient-to-br from-emerald-50 via-white to-teal-50">
          ...
          >
            <motion.div variants={fadeInUp} className="space-y-4">
              ...
              <h2 className="text-5xl md:text-6xl font-bold text-foreground">Hire AI agents that <span className="text-emerald-500">actually do the work</span></h2>
              <p className="text-xl text-muted-foreground max-w-3xl">Inside one unified workspace. No more tool hopping. No more hiring headaches.</p>
            </motion.div>
            ...
            <motion.div variants={staggerContainer} className="grid md:grid-cols-3 gap-6">
              {[
                ...
                <motion.div key={i} variants={scaleIn} className="bg-card border border-emerald-200 rounded-2xl p-6 hover:border-emerald-300 transition-all shadow-sm">
                  ...
                  <div className="pt-4 border-t border-border mt-4"><div className="text-muted-foreground text-sm">{item.metric}</div></div>
                </motion.div>
              ))}
            </motion.div>
            
            <motion.div variants={fadeInUp} className="bg-gradient-to-r from-emerald-50 to-teal-50 border border-emerald-200 rounded-2xl p-8 text-center shadow-sm">
              <div className="text-2xl text-foreground font-semibold mb-2">"It's like having a team of specialists, without the team"</div>
        ...
        <section className="min-h-screen flex flex-col justify-center py-20 px-6 relative print:break-after-page bg-gradient-to-br from-amber-50 via-white to-orange-50">
          ...
          >
            <motion.div variants={fadeInUp} className="space-y-4">
              ...
              <h2 className="text-5xl md:text-6xl font-bold text-foreground">Why <span className="text-amber-500">now?</span></h2>
            </motion.div>
            
            <motion.div variants={staggerContainer} className="grid md:grid-cols-2 gap-6">
              {[
                { icon: Brain, title: "LLMs Can Now Execute", desc: "GPT-4, Claude 3, Gemini—models finally capable of complex reasoning and tool use", stat: "100x better than 2022" },
                { icon: Workflow, title: "Tool Calling Is Native", desc: "Function calling, structured outputs, and API orchestration are now first-class features", stat: "Reliable agentic loops" },
                { icon: DollarSign, title: "Inference Costs Crashed", desc: "10x cheaper than 18 months ago, making multi-agent systems economically viable", stat: "~$0.002 per task" },
                { icon: Globe, title: "Market Adoption Ready", desc: "80% of businesses now use AI tools. The jump to AI workers is a small step.", stat: "Mainstream acceptance" }
              ].map((item, i) => (
                <motion.div key={i} variants={scaleIn} className="bg-card border border-amber-200 rounded-2xl p-6 hover:border-amber-300 transition-all shadow-sm group">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="inline-block px-3 py-1 bg-amber-100 rounded-full text-amber-700 text-sm">{item.stat}</div>
                    </div>
                  </div>
                </motion.div>
              ))}
            ...
            <motion.div variants={fadeInUp} className="text-center">
              <div className="inline-block bg-gradient-to-r from-amber-100 to-orange-100 border border-amber-200 rounded-full px-8 py-4 shadow-sm">
                <span className="text-xl text-foreground">The <span className="text-amber-600 font-semibold">infrastructure exists</span>. The <span className="text-amber-600 font-semibold">market is ready</span>. We're building the <span className="text-amber-600 font-semibold">interface</span>.</span>
              </div>
            </motion.div>
          </motion.div>
        </section>
        ...
        <section className="min-h-screen flex flex-col justify-center py-20 px-6 relative print:break-after-page bg-gradient-to-br from-violet-50 via-white to-purple-50">
          ...
          >
            <motion.div variants={fadeInUp} className="space-y-4">
              ...
              <h2 className="text-5xl md:text-6xl font-bold text-foreground">The <span className="text-violet-500">Wow</span> Slide</h2>
              <p className="text-xl text-muted-foreground">Watch a complete workflow: request → plan → execute → deliver</p>
            </motion.div>
            
            <motion.div variants={scaleIn} className="bg-card backdrop-blur-xl border border-violet-200 rounded-3xl p-8 shadow-lg">
              ...
              <div className="space-y-4">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1 bg-muted/50 rounded-xl p-4 border border-border">
                    ...
                    <div className="text-foreground">"Create a weekly social media report and post engagement summary to Slack"</div>
                  </div>
                </div>
                
                <div className="flex items-start gap-4">
                  <div className="w-10 h-10 rounded-full bg-violet-100 flex items-center justify-center text-violet-600 font-bold shrink-0">2</div>
                  <div className="flex-1 bg-violet-50 rounded-xl p-4 border border-violet-200">
                    ...
                    <div className="text-foreground/80 space-y-1 text-sm">
                      {["Connect to social platforms", "Pull engagement metrics", "Generate visual report", "Post to #marketing-updates"].map((step, i) => (
                        <div key={i} className="flex items-center gap-2"><ChevronRight className="w-4 h-4 text-violet-500" />{step}</div>
                      ))}
                    </div>
                  </div>
                </div>
                  ...
                  <div className="flex-1 bg-emerald-50 rounded-xl p-4 border border-emerald-200">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {['Instagram API', 'Twitter API', 'LinkedIn API', 'Chart Gen', 'Slack Webhook'].map((tool) => (
                        <span key={tool} className="px-3 py-1 bg-emerald-100 rounded-full text-emerald-700 text-xs flex items-center gap-1">
                          <CheckCircle2 className="w-3 h-3" />{tool}
                        </span>
                      ))}
                    </div>
                  </div>
                ...
                <div className="flex items-start gap-4">
                  <div className="w-10 h-10 rounded-full bg-amber-100 flex items-center justify-center text-amber-600 font-bold shrink-0">4</div>
                  <div className="flex-1 bg-amber-50 rounded-xl p-4 border border-amber-200">
                    ...
                    <div className="grid grid-cols-3 gap-4 text-center">
                      <div className="bg-card rounded-lg p-3 border border-border"><div className="text-2xl font-bold text-foreground">+23%</div><div className="text-xs text-muted-foreground">Engagement</div></div>
                      <div className="bg-card rounded-lg p-3 border border-border"><div className="text-2xl font-bold text-foreground">12.4k</div><div className="text-xs text-muted-foreground">Impressions</div></div>
                      <div className="bg-card rounded-lg p-3 border border-border"><div className="text-2xl font-bold text-foreground">847</div><div className="text-xs text-muted-foreground">New Followers</div></div>
                    </div>
                  </div>
                </div>
                
                <div className="flex items-start gap-4">
                  <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold shrink-0">5</div>
                  <div className="flex-1 bg-blue-50 rounded-xl p-4 border border-blue-200">
                    ...
                    <div className="flex items-center gap-4">
                      <button className="px-4 py-2 bg-emerald-500 rounded-lg text-white text-sm font-medium flex items-center gap-2"><CheckCircle2 className="w-4 h-4" />Approve & Post</button>
                      <button className="px-4 py-2 bg-muted rounded-lg text-foreground text-sm border border-border">Edit</button>
                      <button className="px-4 py-2 bg-muted rounded-lg text-foreground text-sm border border-border">Schedule</button>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <motion.div variants={fadeInUp} className="text-center text-muted-foreground">Total time: <span className="text-violet-600 font-semibold">47 seconds</span> — what used to take an intern 3 hours</motion.div>
          </motion.div>
        </section>
        ...
        <section className="min-h-screen flex flex-col justify-center py-20 px-6 relative print:break-after-page bg-gradient-to-br from-cyan-50 via-white to-sky-50">
          ...
          >
            <motion.div variants={fadeInUp} className="space-y-4">
              ...
              <h2 className="text-5xl md:text-6xl font-bold text-foreground">AI System + <span className="text-cyan-500">Trust</span></h2>
              <p className="text-xl text-muted-foreground max-w-2xl">Enterprise-grade safety with consumer-grade simplicity</p>
            </motion.div>
            
            <div className="grid md:grid-cols-2 gap-8">
              <motion.div variants={slideInLeft} className="bg-card border border-cyan-200 rounded-2xl p-6 shadow-sm">
                <h3 className="text-xl font-semibold text-foreground mb-6 flex items-center gap-2"><Layers className="w-5 h-5 text-cyan-500" />High-Level Architecture</h3>
                <div className="space-y-3">
                  {[
                    ...
                    <div key={i} className="flex items-center gap-4 p-3 bg-cyan-50 rounded-lg border-l-4 border-cyan-500">
                      <div><div className="text-foreground font-medium">{item.label}</div><div className="text-muted-foreground text-sm">{item.desc}</div></div>
                    </div>
                  ))}
                </div>
              </motion.div>
              ...
              <motion.div variants={slideInRight} className="bg-card border border-cyan-200 rounded-2xl p-6 shadow-sm">
                <h3 className="text-xl font-semibold text-foreground mb-6 flex items-center gap-2"><Shield className="w-5 h-5 text-cyan-500" />Safety & Controls</h3>
                <div className="space-y-4">
                  <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-lg">
                    <div className="text-emerald-600 font-semibold mb-2 flex items-center gap-2"><Check className="w-4 h-4" />What Agents CAN Do</div>
                  ...
                  <div className="p-4 bg-rose-50 border border-rose-200 rounded-lg">
                    <div className="text-rose-600 font-semibold mb-2 flex items-center gap-2"><X className="w-4 h-4" />What Agents CAN'T Do</div>
                    <ul className="text-foreground/70 text-sm space-y-1">
            ...
            <motion.div variants={staggerContainer} className="grid grid-cols-3 gap-4">
              {[
                ...
                <motion.div key={i} variants={scaleIn} className="text-center p-4 bg-card rounded-xl border border-border shadow-sm">
        ...
        <section className="min-h-screen flex flex-col justify-center py-20 px-6 relative print:break-after-page bg-gradient-to-br from-pink-50 via-white to-rose-50">
          ...
          >
            <motion.div variants={fadeInUp} className="space-y-4">
              ...
              <h2 className="text-5xl md:text-6xl font-bold text-foreground">Market <span className="text-pink-500">Size</span></h2>
            </motion.div>
            
            <div className="grid md:grid-cols-2 gap-8">
              <motion.div variants={slideInLeft} className="bg-card border border-pink-200 rounded-2xl p-6 shadow-sm">
                <h3 className="text-xl font-semibold text-foreground mb-6 flex items-center gap-2"><Target className="w-5 h-5 text-pink-500" />Ideal Customer Profile</h3>
                <div className="space-y-3">
                  {[
                    { segment: "E-commerce Operators", size: "2M+", pain: "Order processing, customer service, inventory" },
                    { segment: "Digital Agencies", size: "500k+", pain: "Client deliverables, reporting, coordination" },
                    ...
                    <div key={i} className="p-4 bg-pink-50/50 rounded-lg border border-border hover:border-pink-300 transition-all">
              ...
              <motion.div variants={slideInRight} className="space-y-4">
                {[
                  { label: "TAM", title: "Total Addressable Market", value: "$340B", desc: "Global enterprise software + BPO", color: "bg-gradient-to-r from-pink-50 to-rose-50 border-pink-200" },
                  { label: "SAM", title: "Serviceable Available Market", value: "$48B", desc: "SMB automation + AI assistants", color: "bg-gradient-to-r from-purple-50 to-violet-50 border-purple-200" },
                  { label: "SOM", title: "Serviceable Obtainable Market", value: "$2.4B", desc: "English-speaking SMBs ready for AI agents", color: "bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200" }
                ].map((item, i) => (
            ...
            <motion.div variants={fadeInUp} className="bg-gradient-to-r from-pink-50 to-purple-50 border border-pink-200 rounded-2xl p-6 text-center shadow-sm">
              <div className="text-muted-foreground text-sm mb-2">5-Year Projection</div>
              <div className="text-3xl font-bold text-foreground">AI agent market growing at <span className="text-pink-500">47% CAGR</span></div>
            </motion.div>
          </motion.div>
        </section>
        ...
        <section className="min-h-screen flex flex-col justify-center py-20 px-6 relative print:break-after-page bg-gradient-to-br from-orange-50 via-white to-amber-50">
          ...
          >
            <motion.div variants={fadeInUp} className="space-y-4">
              ...
              <h2 className="text-5xl md:text-6xl font-bold text-foreground">Alternatives vs <span className="text-orange-500">Elixa</span></h2>
            </motion.div>
            
            <motion.div variants={scaleIn} className="overflow-x-auto bg-card rounded-2xl border border-border shadow-sm">
                ...
                <tbody>
                  {[
                    ...
                    <tr key={i} className={`border-b border-border ${row.highlight ? 'bg-orange-50' : ''}`}>
                      <td className={`py-4 px-6 ${row.highlight ? 'text-orange-600 font-semibold' : 'text-foreground'}`}>{row.name}</td>
                      {row.vals.map((v, j) => (
                        <td key={j} className="text-center py-4 px-4">
                          {v ? <Check className="w-5 h-5 text-emerald-500 mx-auto" /> : <X className="w-5 h-5 text-muted-foreground/30 mx-auto" />}
                        </td>
                      ))}
                    </tr>
                  ))}
            ...
            <motion.div variants={fadeInUp} className="bg-gradient-to-r from-orange-50 to-amber-50 border border-orange-200 rounded-2xl p-8 shadow-sm">
        ...
        <section className="min-h-screen flex flex-col justify-center py-20 px-6 relative print:break-after-page bg-gradient-to-br from-green-50 via-white to-emerald-50">
          ...
          >
            <motion.div variants={fadeInUp} className="space-y-4">
              ...
              <h2 className="text-5xl md:text-6xl font-bold text-foreground">Simple, <span className="text-green-500">Scalable</span> Pricing</h2>
            </motion.div>
            
            <motion.div variants={staggerContainer} className="grid md:grid-cols-3 gap-6">
              {[
                { tier: "Starter", price: "£20", agents: "3 agents", extra: "£10 per extra agent", features: ["Core agent capabilities", "5GB storage", "Email support"], popular: false },
                { tier: "Growth", price: "£35", agents: "6 agents", extra: "£7 per extra agent", features: ["Everything in Starter", "25GB storage", "Priority support", "Advanced analytics"], popular: true },
                { tier: "Scale", price: "£50", agents: "10 agents", extra: "£5 per extra agent", features: ["Everything in Growth", "100GB storage", "Dedicated success manager", "Custom integrations"], popular: false }
              ].map((plan, i) => (
                <motion.div key={i} variants={scaleIn} className={`relative bg-card border rounded-2xl p-6 shadow-sm ${plan.popular ? 'border-green-400 ring-2 ring-green-200' : 'border-border'}`}>
                  {plan.popular && <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-green-500 rounded-full text-white text-xs font-semibold">Most Popular</div>}
                  <div className="text-center mb-6">
                    <div className="text-muted-foreground text-sm">{plan.tier}</div>
                    <div className="text-4xl font-bold text-foreground mt-1">{plan.price}<span className="text-lg font-normal text-muted-foreground">/mo</span></div>
                  ...
                  <ul className="space-y-2">
                    {plan.features.map((feature, j) => (
                      <li key={j} className="flex items-center gap-2 text-foreground/80 text-sm"><Check className="w-4 h-4 text-green-500" />{feature}</li>
                    ))}
                  </ul>
                </motion.div>
              ))}
        ...
        <section className="min-h-screen flex flex-col justify-center py-20 px-6 relative print:break-after-page bg-gradient-to-br from-indigo-50 via-white to-violet-50">
          ...
          >
            <motion.div variants={fadeInUp} className="space-y-4">
              ...
              <h2 className="text-5xl md:text-6xl font-bold text-foreground">Phased <span className="text-indigo-500">Expansion</span></h2>
            </motion.div>
            
            <motion.div variants={staggerContainer} className="grid md:grid-cols-3 gap-6">
              {[
                { phase: "Phase 1", timeline: "Now - Q2 2025", title: "Core Revenue", items: ["Agent slot subscriptions", "Storage add-ons", "Usage-based pricing"], revenue: "Primary MRR driver", color: "border-indigo-200 bg-indigo-50/50" },
                { phase: "Phase 2", timeline: "Q3 - Q4 2025", title: "Marketplace Revenue", items: ["Developer subscriptions", "Agent marketplace fees", "Premium agent listings"], revenue: "Platform take rate", color: "border-purple-200 bg-purple-50/50" },
                { phase: "Phase 3", timeline: "2026+", title: "Enterprise Expansion", items: ["Workspace tiering", "Slack/Teams native apps", "Enterprise contracts"], revenue: "Higher ACV deals", color: "border-blue-200 bg-blue-50/50" }
              ].map((phase, i) => (
                <motion.div key={i} variants={scaleIn} className={`bg-card border ${phase.color} rounded-2xl p-6 shadow-sm hover:shadow-md transition-all`}>
                  ...
                  <ul className="space-y-2 mb-4">
                    {phase.items.map((item, j) => (
                      <li key={j} className="flex items-center gap-2 text-foreground/80 text-sm"><ChevronRight className="w-4 h-4 text-indigo-500" />{item}</li>
                    ))}
                  </ul>
                  <div className="pt-4 border-t border-border"><span className="text-muted-foreground text-sm">{phase.revenue}</span></div>
                </motion.div>
              ))}
            </motion.div>
          </motion.div>
        ...
        <section className="min-h-screen flex flex-col justify-center py-20 px-6 relative print:break-after-page bg-gradient-to-br from-rose-50 via-white to-pink-50">
          ...
          >
            <motion.div variants={fadeInUp} className="space-y-4">
              ...
              <h2 className="text-5xl md:text-6xl font-bold text-foreground">Go-to-<span className="text-rose-500">Market</span></h2>
            </motion.div>
            
            <div className="grid md:grid-cols-2 gap-8">
              <motion.div variants={slideInLeft} className="bg-card border border-rose-200 rounded-2xl p-6 shadow-sm">
                <h3 className="text-xl font-semibold text-foreground mb-6 flex items-center gap-2"><Rocket className="w-5 h-5 text-rose-500" />Acquisition Channels</h3>
                <div className="space-y-3">
                  {[
                    { channel: "Founder-Led Outbound", desc: "Direct outreach to ICP via LinkedIn, email", conversion: "12% reply rate" },
                    { channel: "Community Building", desc: "Twitter/X, Reddit, Indie Hackers, Discord", conversion: "Organic growth engine" },
                    { channel: "Strategic Partnerships", desc: "Agency partnerships, tech integrations", conversion: "Warm referrals" },
                    { channel: "Platform Integrations", desc: "Shopify App Store, Slack Directory", conversion: "Discovery at point of need" }
                  ].map((item, i) => (
                    <div key={i} className="p-4 bg-rose-50/50 rounded-lg border border-border hover:border-rose-300 transition-all">
                      <div className="flex items-center justify-between mb-1">
              ...
              <motion.div variants={slideInRight} className="bg-card border border-rose-200 rounded-2xl p-6 shadow-sm">
                <h3 className="text-xl font-semibold text-foreground mb-6 flex items-center gap-2"><TrendingUp className="w-5 h-5 text-rose-500" />Conversion Flywheel</h3>
                <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-3 gap-4 text-center">
                    <div className="p-3 bg-muted/50 rounded-lg"><div className="text-2xl font-bold text-foreground">72%</div><div className="text-muted-foreground text-xs">Day 1 activation</div></div>
                    <div className="p-3 bg-muted/50 rounded-lg"><div className="text-2xl font-bold text-foreground">3.2</div><div className="text-muted-foreground text-xs">Avg agents/user</div></div>
                    <div className="p-3 bg-muted/50 rounded-lg"><div className="text-2xl font-bold text-foreground">40%</div><div className="text-muted-foreground text-xs">Add agent week 2</div></div>
                  </div>
                </div>
              </motion.div>
            </div>
            
            <motion.div variants={fadeInUp} className="bg-gradient-to-r from-rose-50 to-pink-50 border border-rose-200 rounded-2xl p-6 text-center shadow-sm">
              <div className="text-xl text-foreground"><span className="text-rose-500 font-semibold">Land & Expand:</span> Start with one agent, prove value, then become the operating system</div>
            </motion.div>
          </motion.div>
        </section>
        ...
        <section className="min-h-screen flex flex-col justify-center py-20 px-6 relative print:break-after-page bg-gradient-to-br from-teal-50 via-white to-cyan-50">
          ...
          >
            <motion.div variants={fadeInUp} className="space-y-4">
              ...
              <h2 className="text-5xl md:text-6xl font-bold text-foreground">Traction & <span className="text-teal-500">Validation</span></h2>
            </motion.div>
            ...
            <motion.div variants={staggerContainer} className="grid md:grid-cols-4 gap-6">
              {[
                ...
                <motion.div key={i} variants={scaleIn} className="bg-card border border-teal-200 rounded-2xl p-6 text-center shadow-sm hover:shadow-md transition-all">
            ...
            <div className="grid md:grid-cols-2 gap-8">
              <motion.div variants={slideInLeft} className="bg-card border border-teal-200 rounded-2xl p-6 shadow-sm">
                <h3 className="text-xl font-semibold text-foreground mb-6 flex items-center gap-2"><BarChart3 className="w-5 h-5 text-teal-500" />Pilot Case Studies</h3>
                <div className="space-y-3">
                  {[
                    { company: "E-commerce Brand (£2M ARR)", result: "60% reduction in order processing time", agent: "Ops Agent" },
                    { company: "Digital Agency (8 person)", result: "3x client report throughput", agent: "Reporting Agent" },
                    { company: "SaaS Startup (Seed)", result: "Saved 20hrs/week on marketing tasks", agent: "Marketing Agent" }
                  ].map((item, i) => (
              ...
              <motion.div variants={slideInRight} className="bg-card border border-teal-200 rounded-2xl p-6 shadow-sm">
                <h3 className="text-xl font-semibold text-foreground mb-6 flex items-center gap-2"><PieChart className="w-5 h-5 text-teal-500" />Usage Metrics</h3>
                <div className="space-y-3">
                  {[
                    ...
                    <div key={i} className="flex items-center justify-between p-3 bg-muted/50 rounded-lg border border-border">
                      <div><div className="text-foreground font-medium">{item.label}</div><div className="text-muted-foreground text-xs">{item.period}</div></div>
            ...
            <div className="grid md:grid-cols-2 gap-12">
              ...
              <motion.div variants={slideInLeft}>
                <div className="space-y-4 mb-8">
                  ...
                  <h2 className="text-4xl md:text-5xl font-bold text-foreground">Why <span className="text-primary">Us</span></h2>
                </div>
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={i} className="flex items-start gap-4 bg-card border border-primary/20 rounded-xl p-5 shadow-sm">
                ...
                <div className="mt-8 p-4 bg-card rounded-xl border border-border shadow-sm">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    {["AI/ML", "SaaS", "Scaling 0→1", "B2B Sales"].map((skill, i) => (
                      <span key={i} className="px-3 py-1 bg-primary/10 rounded-full text-primary text-sm">{skill}</span>
                    ))}
                  </div>
                </div>
              </motion.div>
              ...
              <motion.div variants={slideInRight}>
                <div className="space-y-4 mb-8">
                  ...
                  <h2 className="text-4xl md:text-5xl font-bold text-foreground">Raising <span className="text-indigo-500">£[X]M</span></h2>
                </div>
                    ...
                    {[
                      ...
                      <div key={i}>
                        ...
                        <div className="w-full bg-muted rounded-full h-2 mb-1">
                          <div className="bg-gradient-to-r from-indigo-500 to-purple-500 h-2 rounded-full" style={{ width: `${item.percent}%` }} />
                        </div>
                ...
                <div className="bg-card border border-border rounded-xl p-6 shadow-sm">
                  ...
                  <div className="space-y-2">
                    {["500 paying customers", "£500k ARR", "20+ agents in marketplace", "Series A ready"].map((milestone, i) => (
                      <div key={i} className="flex items-center gap-3 text-foreground/80">
                        <CheckCircle2 className="w-5 h-5 text-indigo-500 shrink-0" />{milestone}
                      </div>
                    ))}
                  </div>
                </div>
