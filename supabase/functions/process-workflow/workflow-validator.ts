...
export class WorkflowValidator {
  ...
  /**
   * Stage 3: Validate executor availability (simplified for upload-time)
   */
  private validateExecutors(workflow: ParsedWorkflow, result: ValidationResult): void {
    console.log(`\n⚙️ Validating node executability...`);
    
    for (const node of workflow.nodes) {
      const definition = this.registry.findDefinition(node.type);
      
      // For upload-time validation, we just check if the node is known and executable
      // We don't validate actual executor implementation (that's for route-to-agent)
      if (definition?.isExecutable) {
        console.log(`  ✓ Executable: ${node.name} (${node.type})`);
      } else if (definition && !definition.isExecutable) {
        console.log(`  - Orchestration: ${node.name} (${node.type})`);
      }
    }
  }
